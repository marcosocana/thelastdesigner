...
const handleMemberCountChange = (e: React.ChangeEvent) => {
const value = e.target.value.replace(/[^1-9]/g, ''); // Solo permite números del 1 al 9
if (value) {
const numericValue = parseInt(value, 10);
setMemberCount(numericValue);

   if (numericValue > memberNames.length) {
    setMemberNames([...memberNames, ...Array(numericValue - memberNames.length).fill("")]);
  } else {
    setMemberNames(memberNames.slice(0, numericValue));
  }
} else {
  setMemberCount(1);
  setMemberNames([""]);
